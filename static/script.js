const CATEGORY_EMOJI = {
  ...
  async function addExpense() {
    ...
    try {
      ...
      if (!res.ok) { showMsg(data.error || "Error adding expense.", "error"); return; }
  ...
  function renderSummary(summary) {
    totalAmountEl.textContent = "₹" + summary.total.toLocaleString("en-IN", { minimumFractionDigits: 2 });
  ...
  function renderList(expenses) {
    ...
    if (!filtered.length) {
      expenseListEl.innerHTML = `<p class="empty-state">${query ? "No results found." : "No expenses recorded yet."}</p>`;
      return;
    }
  
    const sorted = [...filtered].sort((a, b) => new Date(b.date) - new Date(a.date));
    expenseListEl.innerHTML = sorted.map(e => `
      <div class="expense-item">
        <div class="expense-emoji">${CATEGORY_EMOJI[e.category] || "📦"}</div>
        <div class="expense-info">
          <div class="expense-title">${escHtml(e.title)}</div>
          <div class="expense-meta">${e.category} · ${formatDate(e.date)}${e.note ? " · " + escHtml(e.note) : ""}</div>
        </div>
        <div class="expense-amount">₹${parseFloat(e.amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</div>
  ...
  function renderBreakdown(byCategory, total) {
    ...
    breakdownEl.innerHTML = entries.map(([cat, amt]) => {
      const pct = total > 0 ? Math.round((amt / total) * 100) : 0;
      return `
        <div class="breakdown-item">
          <div class="breakdown-row">
            <span class="breakdown-cat">${CATEGORY_EMOJI[cat] || "📦"} ${cat}</span>
            <span class="breakdown-amt">₹${parseFloat(amt).toLocaleString("en-IN", { minimumFractionDigits: 2 })} <span style="color:var(--text-dim);font-size:10px">${pct}%</span></span>
          </div>
